...
/**
 * Исправленная функция импорта с двойной защитой формата
 */
function importStudents_(p,user) {
  let students = p.students;

  // Если пришла строка (из-за JSON.stringify на клиенте) — парсим её
  if (typeof students === 'string') {
    try {
      students = JSON.parse(students);
    } catch(e) {
      return json_({success:false, error:'Неверный формат JSON в поле students.'});
    }
  }

  if (!Array.isArray(students) || !students.length) {
    return json_({success:false, error:'Данные учеников пусты или переданы в неверном формате.'});
  }
  ...
  try {
    ...
    if(out.length) {
      s.getRange(s.getLastRow() + 1, 1, out.length, 3).setValues(out);
    }
